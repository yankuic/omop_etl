insert into dbo.visit_detail with (tablock)
insert into dbo.visit_detail with (tablock)
select visit_detail_id = b.visit_detail_id
      ,person_id = c.person_id
      ,visit_detail_concept_id = g.target_concept_id
      ,visit_detail_start_date = a.ENCOUNTER_EFFECTIVE_DATE
      ,visit_detail_start_datetime = a.ENCOUNTER_EFFECTIVE_DATE
      ,visit_detail_end_date = isnull(a.DISCHG_DATE, a.ENCOUNTER_EFFECTIVE_DATE)
      ,visit_detail_end_datetime = isnull(a.DISCHG_DATETIME, a.ENCOUNTER_EFFECTIVE_DATE)
...
--      ,a.patient_type,a.encounter_type,a.hospital,a.location_of_service,a.ed_episode_id
      ,visit_detail_source_value = case
...
--      ,visit_source_concept_id = g.source_concept_id
	  ,visit_detail_source_concept_id = case
	  ...
	  ,visit_occurrence_id = b.visit_occurrence_id
from stage.visit_hospital vh
join stage.visit a on vh.patnt_encntr_key=a.patnt_encntr_key 
