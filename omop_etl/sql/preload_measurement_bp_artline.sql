...
;with bp_artline as (
	...
	select patient_key
            ...
            ,bp_datetime
            ,bp_artline
            ,[bp - art line sbp] = try_convert(int, substring((bp_artline),1,charindex('/',(bp_artline),1)-1))
            ,[bp - art line dbp] = try_convert(int, substring((bp_artline), charindex('/',(bp_artline),1)+1, {fn length((bp_artline))}-charindex('/',(bp_artline),1)))
            ,provider = isnull(attending_provider, visit_provider)
      from stage.MEASUREMENT_BP_ArterialLine
      where bp_artline like '%/%' --accept only values with format ##/##
)
SELECT patient_key
      ,patnt_encntr_key
      ,bp_date
  ...
  FROM bp_artline
cross apply (
      values ('bp - art line sbp', [bp - art line sbp], bp_artline)
            ,('bp - art line dbp', [bp - art line dbp], bp_artline)
...
select person_id = b.person_id
      ...
      ,source_table = 'measurement_bp_arterialline'
