...
SELECT [patient_key]
  ...
  FROM stage.MEASUREMENT_Res_VentPeds
  cross apply (
      values ('VENT START - Peds', respiratory_vent_bgn)
            ,('VENT STOP - Peds', respiratory_vent_end)
...
select person_id = b.person_id
    ...
    ,source_table = 'measurement_res_ventpeds'
...
select person_id = b.person_id
    ...
    ,source_table = 'measurement_res_ventpeds'
from stage.MEASUREMENT_Res_VentPeds a 
join xref.person_mapping b
on a.patient_key = b.patient_key
left join xref.provider_mapping c 
