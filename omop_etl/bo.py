...
def bo_query(doc_name:str, con:object): 
    """Retrieve SQL query generated by BO.
    
    Arguments:
        doc_name {str} -- BO .wid document name.
...
def format_stage_query(doc_name:str, dp_name:str, start_date:str, end_date:str, bo_con:object, con:object, loinc_list:list='', schema:str='stage', aliases:list=None):
    """Format stage query:
	      Replace placeholders for the lsit of patients, start and end datas for pulling data, and list of LOINC codes.
	      See format_bo_sql method for additional formating.
	"""	
...
def format_bo_sql(sqlstring:str, dp_name:str, database:str, schema:str='cohort', aliases:list=None):
    ...
    parsed = sqlparse.parse(sqlstring)[0]  #sql string from BO query with no changes 
    ...
    idx = [parsed.token_index(t) for t in parsed if t.is_keyword and t.value == 'FROM'][0] #number of items/columns between keyword and word 'from'
    ...
    return sqlstring.replace("'", "''")  #ensure to return a string rather than an object
