...
SELECT COALESCE(@TempMaxRowId, @ObsMaxRowId, 0) + row_number() over(ORDER BY a.person_id) AS observation_id ,
       ...
       NULL AS visit_occurrence_id ,
       NULL AS visit_detail_id ,
       ISNULL(b.value_concept_code, b.naaccr_item_number) AS observation_source_value ,
       ISNULL(b.value_concept_id, c.concept_id) AS observation_source_concept_id ,
       NULL AS unit_source_value ,
       NULL AS qualifier_source_value ,
	    -- , NULL AS observation_event_id
		-- , NULL AS obs_event_field_concept_id
...
WHERE b.value_concept_id is not null
;
